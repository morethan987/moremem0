...
- 前端Docker封装；还是无法解决http混合请求不安全问题😢；可能需要刷新一下缓存，前端的源代码使用的是http（搜索memor）可以看到

- 成功改进前端的js文件，直接用https访问服务器接口，但是不知道为什么被降级为了http，浏览器还是报错😢

- 服务器配置成功，但是记忆功能因为Qdrant参数变更而无法使用，全局搜索TODO:ERORR
